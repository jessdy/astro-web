---
title: 从0开始制作docker版ImmortalWrt
description: 这篇文章将引导你从0开始部署安装ImmortalWrt，一个基于OpenWrt的开源路由固件。
duration: 3min
date: 2025-10-14
tags:
  - 路由
...
> 很多人家里有一些闲置的X86的电脑，如果恰好功耗较低，又恰好配置不高，可以用来做旁路由，从而实现网络分流和优化。这里介绍如何从0开始制作docker版ImmortalWrt（当然你也可以使用路由器或其他设备）。

## 什么是ImmortalWrt？

ImmortalWrt是一个基于OpenWrt的开源路由固件，它提供了强大的路由功能和灵活的定制选项。与传统的路由固件不同，ImmortalWrt强调稳定性、安全性和可定制性，适用于各种网络环境和应用场景。

## 0. 准备工作

在x86设备上先安装好操作系统，并安装好docker和docker-compose。

这里是用的ubuntu 22.04系统。

## 1. 下载并解压ImmortalWrt包

打开[ImmortalWrt的官方下载网站](https://downloads.immortalwrt.org/) 或者 [Firmware Selector](https://firmware-selector.immortalwrt.org/)

选择对应的基带版本，这里选择`x86-64`。

下载最新版本的ImmortalWrt包，选择rootfs.tar.gz。然后解压。

```bash
mkdir /opt/immortalwrt
cd /opt/immortalwrt
wget https://downloads.immortalwrt.org/releases/24.10.3/targets/x86/64/immortalwrt-24.10.3-x86-64-rootfs.tar.gz
gzip -d immortalwrt-24.10.3-x86-64-rootfs.tar.gz
```

## 2. 构建ImmortalWrt的docker镜像

在ImmortalWrt目录下创建一个Dockerfile文件，内容如下：
...
然后构建docker镜像，标签为`immortalwrt:24.10.3`，在Dockerfile所在目录执行：

```bash
docker build -t immortalwrt:24.10.3 .
```

构建完成以后可以通过`docker images`查看镜像是否构建成功。

```bash
docker images
```

## 3. 运行ImmortalWrt的docker容器
...
Docker 中创建一个 macvlan 网络，假设 eth0 是宿主机的网卡，网段为 192.168.1.0/24，网关为 192.168.1.1。
...
在`/opt/immortalwrt`目录下创建一个network文件，用于挂载到ImmortalWrt容器的`/etc/config/network`。
...
config interface 'lan'
        ...
        option dns '223.5.5.5 8.8.4.4 223.6.6.6 8.8.8.8'
```

下面用docker命令和docker-compose两种方法分别运行ImmortalWrt容器，二选一即可。
1. docker 命令
```bash
docker run --restart always --name ImmortalWrt \
    -d --network macnet \
    --privileged \
    --ip 192.168.1.254 \
    -v /opt/immortalwrt/network:/etc/config/network \
    immortalwrt:24.10.3 /sbin/init
```
`ImmortalWrt` 容器名称
`192.168.1.254` 容器IP地址，旁路由的IP地址

2. docker-compose 方式
在`/opt/immortalwrt`目录下创建一个docker-compose.yml文件，内容如下：

```yaml
version: '3.8'
services:
  openwrt:
    image: immortalwrt:24.10.3
    ...
    volumes:
      - /opt/immortalwrt/network:/etc/config/network # 挂载宿主机的目录到容器
    command: /sbin/init # 指定容器启动命令
...
安装完ImmortalWrt后，你可以通过`192.168.1.254`访问ImmortalWrt的Web界面，初次登陆会要求你设置管理员密码。

下面介绍一些实用的软件：

1. iStore是一个基于OpenWrt的插件商店，你可以在其中安装各种插件，如DDNS、VPN、防火墙等。
```bash
